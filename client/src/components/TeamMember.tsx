...
export default function TeamMember({ name, role, affiliation, expertise, email, image }: TeamMemberProps) {
  const initials = name.split(' ').map(n => n[0]).join('');

  return (
    <Card className="hover-elevate h-full" data-testid="card-team-member">
      ...
      <CardContent className="space-y-3">
        ...
        {email && (
          <a href={`mailto:${email}`} className="flex items-center gap-2 text-sm text-primary hover-elevate active-elevate-2 p-2 rounded-md -ml-2" data-testid="link-member-email">
